...
def create_train(images, labels, n_labeled, wanted_labels, pure, seed=0):
    wanted_indices = []
    for label in wanted_labels:
        if len(labels.shape) == 2:
            indices = np.where(labels[:, 0] == label)
    ...
    if n_labeled == len(images):
        ...
        if pure: # if pure, the images belong to the unwanted classes are ignored
            images = torch.cat([l_imgs, rest_imgs], 0)
            labels = torch.cat([l_labels, rest_labels], 0)
        else:
